...
clip_scales = [
    ...
    2.2,
    2.8,
]  # possible clip scales order by diversty
...
class DataDict:
    def __init__(self, data_dict):
        ...
        self.predicate_pcl_flag = np.array(data_dict["predicate_pcl_flag"]).astype(
            np.float32
        )
...
def _load_data_tqdm(shared_list, relationship):
    ...
    path = os.path.join(
        CONF.PATH.R3SCAN,
        "preprocessed",
        "{}/data_dict_{}.pkl".format(
            relationship["scan"], str(hex(relationship["split"]))[-1]
        ),
    )
    try:
...
class Open2D3DSGDataset(Dataset):
    ...
    def __init__(
        ...
        if self.relationships_R3SCAN:
            self.obj_vis_crit -= (
                0.1  # r3scan images are smaller than scannet lets adjust
            )
            self.obj_mask_crit -= 0.1
            self.rel_vis_crit -= 0.1
            # Load all data into ram for faster training
            process_map(
                partial(_load_data_tqdm, shared_list),
                self.relationships_R3SCAN,
                max_workers=8,
                chunksize=1,
            )
        ...
        if self.relationships_scannet:
            # Load all data into ram for faster training
            process_map(
                partial(_load_data_scannet_tqdm, shared_list),
                self.relationships_scannet,
                max_workers=8,
                chunksize=1,
            )
        ...
        for i, (k, v) in enumerate(obj2frame.items()):
            ...
            if dataset == "scannet":
                imgs = [
                    Image.open(
                        os.path.join(
                            CONF.PATH.SCANNET_RAW, "scannet_2d", scene_id, "color", s[1]
                        )
                    )
                    for s in selected
                ]
                ...
                imgs = [
                    Image.open(
                        os.path.join(CONF.PATH.R3SCAN_RAW, scene_id, "sequence", s[1])
                    ).resize((224, 172))
                    for s in selected
                ]

            cropped_imgs = [
                img.crop(scale_bbox(s[2], sc, img))
                for img, s in zip(imgs, selected)
                for sc in clip_scales[:scales]
            ]
            if dataset == "3rscan":
                cropped_imgs = [img.rotate(-90, expand=True) for img in cropped_imgs]

            if not np.array(
                [((img.size[0] > 0) & (img.size[1] > 0)) for img in cropped_imgs]
            ).all():
                print("zero area")
            processed_imgs = torch.stack(
                [self.preprocess_obj(im) for im in cropped_imgs], dim=0
            )

            if len(cropped_imgs) < (scales) * top_k:
                blanks = torch.zeros(
                    (
                        (scales) * top_k - len(cropped_imgs),
                        3,
                        self.img_dim,
                        self.img_dim,
                    )
    ...
    def obj_pixel_selection(self, obj2frame, scene_id, dataset, top_k=3):
        ...
        for i, (k, v) in enumerate(obj2frame.items()):  # range(len(obj2frame)):
            obj = v  # obj2frame_lst[i]
            frames, pixels, vis, bbox, pixel_ids = tuple(
                np.array(t, dtype=object) for t in zip(*obj)
            )

            if dataset == "3rscan":
                bbox = np.array(
                    (172 - bbox[:, 1], bbox[:, 0], 172 - bbox[:, 3], bbox[:, 2])
                ).T
                pixel_ids = np.array(
                    [
                        np.array([pids[:, 1], 172 - pids[:, 0]], dtype=object).T
                        for pids in pixel_ids
                    ],
                    dtype=object,
                )
                pixel_ids = np.array(
                    [
                        pids[(pids[:, 1] < 172) & (pids[:, 0] < 224)]
                        for pids in pixel_ids
                    ],
                    dtype=object,
                )
            ...
            vis_criterion = (vis > self.obj_mask_crit) | (
                (bbox[:, 2] - bbox[:, 0]) * (bbox[:, 3] - bbox[:, 1])
                > 8000 * r3scan_bias
            )
            if vis_criterion.sum() > 1:
            ...
            if dataset == "scannet":
                ...
                imgs = [
                    np.asarray(
                        Image.open(
                            os.path.join(
                                CONF.PATH.R3SCAN_RAW, scene_id, "sequence", s[1]
                            )
                        )
                        .resize((224, 172))
            ...
            if not np.array(
                [((img.shape[0] > 0) & (img.shape[1] > 0)) for img in imgs]
            ).all():
                print("zero area")
    ...
    def rel_frame_selection(
        self, rel2frame, rel2frame_mask, scene_id, dataset, top_k=4, scales=2
    ):
        reference_imgs = []
        ...
        for i, rel in enumerate(rel2frame.values()):
            ...
            if len(rel) == 0:
                reference_imgs.append(
                    torch.zeros(
                        ((scales) * top_k, 3, self.rel_img_dim, self.rel_img_dim)
                    )
                )
                continue
            frames, s_pixels, o_pixels, s_vis, o_vis, s_bbox, o_bbox = tuple(
                np.array(t) for t in zip(*rel)
            )
            ...
            vis_criterion = (
                (o_vis > self.rel_vis_crit) | (o_A > 10000 * r3scan_bias)
            ...
            if vis_criterion.sum() > 1:
                ...
                if (
                    ...
                    s_pix_norm = (
                        s_pixels[vis_criterion] - np.min(s_pixels[vis_criterion])
                    ) / (
                        np.max(s_pixels[vis_criterion])
                        - np.min(s_pixels[vis_criterion])
                    )
                if (
                    ...
                    o_pix_norm = (
                        o_pixels[vis_criterion] - np.min(o_pixels[vis_criterion])
                    ) / (
                        np.max(o_pixels[vis_criterion])
                        - np.min(o_pixels[vis_criterion])
                    )
            ...
            cropped_imgs = [
                img.crop(scale_bbox(enclosing_bbox(s[2], s[3]), 1 + (sc - 1) / 2, img))
                for img, s in zip(imgs, selected)
                for sc in clip_scales[:scales]
            ]
            if dataset == "3rscan":
                cropped_imgs = [img.rotate(-90, expand=True) for img in cropped_imgs]
            processed_imgs = torch.stack(
                [self.preprocess_rel(im) for im in cropped_imgs], dim=0
            )
    ...
    def blip_rel_frames(
        ...
        for i, rel in enumerate(rel2frame.values()):
            ...
            vis_criterion = (
                (o_vis > self.rel_vis_crit / 2) | (o_A > 10000 * r3scan_bias)
    ...
    def load_imgs(self, data_dict):
        ...
        data_dict["obj2frame_mask"] = np.array(list(obj2frame_mask.values())).astype(
            float
        )
        padding_width = ((0, self.max_objs - data_dict["obj2frame_mask"].shape[0]),)
        data_dict["obj2frame_mask"] = np.pad(
            data_dict["obj2frame_mask"],
            padding_width,
            mode="constant",
            constant_values=0,
        )
        ...
        if self.openseg:
            obj_raw_imgs, obj2frame_raw_mask, obj_frame_pixels = (
                self.obj_pixel_selection(
                    data_dict["obj2frame"],
                    data_dict["scene_id"],
                    data_dict["dataset"],
                    top_k=self.top_k_frames,
            ...
            data_dict["object_raw_imgs"] = np.zeros(
                (self.max_objs, *obj_raw_imgs.shape[1:])
            )
            data_dict["object_raw_imgs"][: len(obj_raw_imgs)] = obj_raw_imgs

            data_dict["obj2frame_raw_mask"] = np.array(
                list(obj2frame_raw_mask.values())
            ).astype(float)
            padding_width = (
                (0, self.max_objs - data_dict["obj2frame_raw_mask"].shape[0]),
            )
            data_dict["obj2frame_raw_mask"] = np.pad(
                data_dict["obj2frame_raw_mask"],
                padding_width,
                mode="constant",
                constant_values=0,
            )
        ...
        if self.blip or self.llava:
            ...
            blank_img_dim = (
                (320, 240) if data_dict["dataset"] == "scannet" else (224, 172)
            )
            black_image = Image.new("RGB", blank_img_dim, (0, 0, 0))
            rel_imgs.extend(
                [[black_image] * self.top_k_frames * self.scales]
                * (self.max_rels - len(rel_imgs))
            )
            data_dict["blip_images"] = rel_imgs

            data_dict["blip_mask"] = rel2frame_mask

            data_dict["rel2frame_mask"] = np.array(
                list(rel2frame_mask.values())
            ).astype(float)
            padding_width = ((0, self.max_rels - data_dict["rel2frame_mask"].shape[0]),)
            data_dict["rel2frame_mask"] = np.pad(
                data_dict["rel2frame_mask"],
                padding_width,
                mode="constant",
                constant_values=0,
            ...
            data_dict["relationship_imgs"] = torch.zeros(
                (self.max_rels, *rel_imgs.shape[1:])
            )
    ...
    def load_features_disk(self, data_dict):
        ...
        torch_numpy = ".pt"
        obj_feature_pth = os.path.join(
            self.load_features, obj_valid_rel_path[0], scan_id + torch_numpy
        )
        obj_valid_feature_pth = os.path.join(
            self.load_features, obj_valid_rel_path[1], scan_id + torch_numpy
        )
        rel_feature_pth = os.path.join(
            self.load_features, obj_valid_rel_path[2], scan_id + torch_numpy
        )
        ...
        data_dict["clip_obj_encoding"] = torch.zeros(
            (self.max_objs, obj_features.shape[-1])
        )
        ...
        if self.blip or self.llava:
            data_dict["clip_rel_encoding"] = torch.zeros(
                (self.max_rels, *rel_features.shape[-2:]), dtype=rel_features.dtype
            )
        else:
            data_dict["clip_rel_encoding"] = torch.zeros(
                (self.max_rels, rel_features.shape[-1])
            )
    ...
    def __getitem__(self, idx):
        ...
        data_dict["objects_id"] = np.pad(
            data_dict["objects_id"].astype(float),
            padding_width,
            mode="constant",
            constant_values=0,
        )
        data_dict["objects_cat"] = np.pad(
            data_dict["objects_cat"].astype(float),
            padding_width,
            mode="constant",
            constant_values=0,
        )
        data_dict["objects_num"] = np.pad(
            data_dict["objects_num"].astype(float),
            padding_width,
            mode="constant",
            constant_values=0,
        )

        data_dict["objects_scale"] = np.pad(
            data_dict["objects_scale"],
            padding_width,
            mode="constant",
            constant_values=0,
        )
        ...
        padding_width = (
            (0, self.max_objs - data_dict["objects_pcl"].shape[0]),
            (0, 0),
            (0, 0),
        )
        data_dict["objects_pcl"] = np.pad(
            data_dict["objects_pcl"], padding_width, mode="constant"
        )
        data_dict["objects_pcl"] = data_dict["objects_pcl"].astype(np.float32)

        padding_width = (
            (0, self.max_objs - data_dict["objects_center"].shape[0]),
            (0, 0),
        )
        data_dict["objects_center"] = np.pad(
            data_dict["objects_center"],
            padding_width,
            mode="constant",
            constant_values=0,
        )
        ...
        padding_width = (
            (0, self.max_rels - data_dict["predicate_cat"].shape[0]),
            (0, 0),
        )
        data_dict["predicate_cat"] = np.pad(
            data_dict["predicate_cat"],
            padding_width,
            mode="constant",
            constant_values=0,
        )
        ...
        data_dict["predicate_dist"] = np.pad(
            data_dict["predicate_dist"],
            padding_width,
            mode="constant",
            constant_values=0,
        )
        data_dict["predicate_dist"] = data_dict["predicate_dist"].astype(np.float32)
        data_dict["predicate_dist"][np.isnan(data_dict["predicate_dist"])] = 0.0

        if "predicate_min_dist" in data_dict.keys():
            data_dict["predicate_min_dist"] = np.pad(
                data_dict["predicate_min_dist"],
                padding_width,
                mode="constant",
                constant_values=0,
            )
            data_dict["predicate_min_dist"] = data_dict["predicate_min_dist"].astype(
                np.float32
            )
            data_dict["predicate_min_dist"][
                np.isnan(data_dict["predicate_min_dist"])
            ] = 0.0

        padding_width = (0, self.max_rels - data_dict["predicate_num"].shape[0])
        data_dict["predicate_num"] = np.pad(
            data_dict["predicate_num"].astype(float),
            padding_width,
            mode="constant",
            constant_values=0,
        )
        ...
        padding_width = (
            (0, self.max_rels - data_dict["predicate_pcl_flag"].shape[0]),
            (0, 0),
            (0, 0),
        )
        data_dict["predicate_pcl_flag"] = np.pad(
            data_dict["predicate_pcl_flag"], padding_width, mode="constant"
        )
        data_dict["predicate_pcl_flag"] = data_dict["predicate_pcl_flag"].astype(
            np.float32
        )

        padding_width = ((0, self.max_rels - data_dict["pairs"].shape[0]), (0, 0))
        data_dict["pairs"] = np.pad(
            data_dict["pairs"].astype(float),
            padding_width,
            mode="constant",
            constant_values=0,
        )
        ...
        data_dict["edges"] = np.pad(
            data_dict["edges"].astype(float),
            padding_width,
            mode="constant",
            constant_values=0,
        )
        ...
        if data_dict["dataset"] == "scannet":
            ...
            objs_bbox = np.array(
                [
                    data_dict["tight_bbox"][i]["axis_aligned"]
                    for i in range(len(data_dict["tight_bbox"]))
                ]
            )
    ...
    def collate_fn(self, data):
        ...
        for key in keys:
            if key == "triples":
                data_dict[key] = [
                    torch.from_numpy(data[i][key]) for i in range(len(data))
                ]
                ...
                data_dict[key] = torch.from_numpy(
                    np.stack([data[i][key] for i in range(len(data))])
                )
        ...
        data_dict["objects_bbox"][:, :, -1] = data_dict["objects_bbox"][:, :, -1] * (
            (2 * np.pi) / 360
        )
        data_dict["objects_bbox"][:, :, -1] = data_dict["objects_bbox"][:, :, -1] % (
            2 * np.pi
        ...
        data_dict["predicate_pcl_flag"][..., 3:6] = (
            data_dict["predicate_pcl_flag"][..., 3:6] / 255
        )
...
if __name__ == "__main__":
    ...
    scans = json.load(
        open(
            os.path.join(
                CONF.PATH.SCANNET, "relationships_validation_filter_wo_misaligned.json"
            )
        )
    )["scans"]
    scans_r3scan = json.load(
        open(os.path.join(CONF.PATH.R3SCAN_RAW, "relationships_validation.json"))
    )["scans"]
    ...
    dataset = Open2D3DSGDataset(
        relationships_scannet=None,
        relationships_R3SCAN=scans_r3scan,
        mini=True,
        img_dim=336,
        top_k_frames=3,
        scales=2,
    ...
    rel_imgs = data_collate["relationship_imgs"][0][
        0
    ]  # batch, pair, view+scale, c, w, h
    ...
    for i in range(len(obj_imgs)):
        ...
        ax.scatter(
            obj_shape[:, 0], obj_shape[:, 1], obj_shape[:, 2], c=obj_shape[:, 3:6]
        )
        set_axes_equal(ax)
        ...
        ax.scatter(
            rel_shape[:, 0], rel_shape[:, 1], rel_shape[:, 2], c=rel_shape[:, -1]
        )
        set_axes_equal(ax)
