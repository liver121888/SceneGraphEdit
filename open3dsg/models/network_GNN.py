...
class MessagePassing_IMP(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_feature):
        """Node"""
        message_pred_to_subj = (
            self.subj_node_gate(torch.cat([x_i, edge_feature], dim=1)) * edge_feature
        )  # n_rel x d
        message_pred_to_obj = (
            self.obj_node_gate(torch.cat([x_j, edge_feature], dim=1)) * edge_feature
        )  # n_rel x d
        node_message = message_pred_to_subj + message_pred_to_obj

        """Edge"""
        message_subj_to_pred = (
            self.subj_edge_gate(torch.cat([x_i, edge_feature], 1)) * x_i
        )  # nrel x d
        message_obj_to_pred = (
            self.obj_edge_gate(torch.cat([x_j, edge_feature], 1)) * x_j
        )  # nrel x d
...
class MessagePassing_VGfM(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_feature, geo_feature):
        ...
        message_geo = (
            self.geo_edge_gate(torch.cat([geo_feature, edge_feature], 1)) * geo_feature
        )
...
class MSG_FAN(MessagePassing):
    def __init__(
        ...
        self.dropout = (
            torch.nn.Dropout(attn_dropout) if attn_dropout > 0 else torch.nn.Identity()
        )

        """update"""
        self.update_node = build_mlp(
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_feature: Tensor) -> Tensor:
        ...
        x_i = self.proj_q(x_i).view(
            num_node, self.dim_node_proj, self.num_head
        )  # [N,D,H]
        edge = self.proj_k(edge_feature).view(
            num_node, self.dim_edge_proj, self.num_head
        )  # [M,D,H]
        x_j = self.proj_v(x_j)
        # est attention
    ...
    def aggregate(
        ...
        inputs[0] = scatter(
            inputs[0], index, dim=self.node_dim, dim_size=dim_size, reduce=self.aggr
        )
        return inputs
...
class JointGNN(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        for _ in range(self.num_layers):
            self.gconvs.append(
                filter_args_create(MSG_FAN, {**kwargs, **kwargs["MSG_FAN"]})
            )
    ...
    def forward(self, x, edge_feature, edge_index):
        ...
        for i in range(self.num_layers):
            gconv = self.gconvs[i]
            if self.with_geo:
                geo_msg = self.geo_gate(
                    torch.cat((node, geo_feature), dim=1)
                ) * torch.sigmoid(
                    geo_feature
                )  # TODO:put the gate back
