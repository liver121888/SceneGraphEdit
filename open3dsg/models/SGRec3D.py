...
class SGRec3D(nn.Module):
    ...
    def _init_encoder(self):
        ...
        if self.pointnet2:
            ...
            self.objPointNet = PointNet2Encoder(
                normal_channel=True
            )  # Input P_i in the paper to predict nodes (phi_n)
            self.relPointNet = PointNet2Encoder(
                normal_channel=True
            )  # Input P_ij in the paper to predict predicate (phi_p)
        else:
            ...
            # The mask is 1 if the point cooresponds to object i, 2 if it cooresponds to object j, 0 otherwise
        ...
        if self.hparams.get("gnn_layers", 0) > 0:
            ...
            if graph_backbone == "message":
                # The GCN model from the paper, takes in t_ij = (phi_n,i, phi_p,ij, phi_n,j) as input
    ...
    def _init_decoder(self):
        ...
        self.node_embedding = build_mlp(
            [num_obj_classes, hidden_dim], activation="relu"
        )
        self.edge_embedding = build_mlp(
            [num_rel_classes, hidden_dim], activation="relu"
        )
    ...
    def encode_pcl(self, objects_pcl, predicate_pcl):
        ...
        objects_pcl_batched = objects_pcl.view(
            -1, *objects_pcl.shape[-2:]
        )  # (batch_size * num_objects, num_points, channels)
        objects_pcl_batched = objects_pcl_batched.permute(
            0, 2, 1
        )  # (batch_size * num_objects, channels, num_points)
    ...
    def encode_gcn(
        self, batch_size, obj_vecs, pred_vecs, objects_count, predicate_count, edges
    ):
        """
        ...
        Args:
            ...
            obj_vecs: Object features [batch_size, num_objects, feature_dim], phi_n in the paper
            pred_vecs: Predicate features [batch_size, num_predicates, feature_dim], phi_p in the paper
        ...
        for i in range(batch_size):
            ...
            if max_nodes > 0:
                o_vecs_out = torch.cat(
                    (
                        o_vecs,
                        torch.zeros((max_nodes - o_vecs.shape[0], o_vecs.shape[1])).to(
                            o_vecs.device
                        ),
                    )
                )
            ...
            if max_edges > 0:
                p_vecs_out = torch.cat(
                    (
                        p_vecs,
                        torch.zeros((max_edges - p_vecs.shape[0], p_vecs.shape[1])).to(
                            p_vecs.device
                        ),
                    )
                )
    ...
    def create_graph_bottleneck(self, node_features, edge_features):
        ...
        edge_bottleneck = torch.sigmoid(
            edge_logits
        )  # Multi-label predicate probabilities
    ...
    def decode_scene(self, node_bottleneck, edge_bottleneck, edges):
        ...
        for i in range(batch_size):
            ...
            boxes_batch = self.box_head(
                node_features_out
            )  # [w, l, h, cx, cy, cz, angle]
            shape_codes_batch = self.shape_head(
                node_features_out
            )  # Shape encoding for AtlasNet
    ...
    def forward(self, data_dict):
        ...
        center_enc, dist_enc = self.encode_center_dist(
            data_dict["objects_center"], data_dict["predicate_dist"]
        )
        ...
        node_bottleneck, edge_bottleneck, node_logits, edge_logits = (
            self.create_graph_bottleneck(obj_vecs_tensor, pred_vecs_tensor)
        )
    ...
    def reconstruction_loss(self, data_dict):
        ...
        angle_pred_logits = torch.zeros(angle_pred.size(0), num_bins).to(
            angle_pred.device
        )
        angle_pred_logits.scatter_(
            1, (angle_pred * num_bins / (2 * np.pi)).long() % num_bins, 1
        )
        angle_gt = (
            torch.round(gt_boxes[..., 6] * num_bins / (2 * np.pi)).long() % num_bins
    ...
    def scene_graph_loss(self, data_dict):
        ...
        obj_loss = self.obj_loss(
            obj_logits.view(-1, obj_logits.size(-1)), obj_gt.view(-1)
        )
...
if __name__ == "__main__":
    ...
    data_dict = {
        ...
        "predicate_pcl_flag": torch.randn(
            batch_size, max_predicates, num_points, channels + 1
        ),
        ...
        "objects_cat": torch.randint(
            0, hparams["num_obj_classes"], (batch_size, max_objects)
        ),
        "predicate_cat": torch.randint(
            0, 2, (batch_size, max_predicates, hparams["num_rel_classes"])
        ).float(),
    }
